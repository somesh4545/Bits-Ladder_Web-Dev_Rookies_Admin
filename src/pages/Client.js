...
const Client = () => {
  ...
  const getUnVerifiedWorkers = () => {
    axios.get("http://localhost:5000/api/v1/admin/clients").then((response) => {
  ...
  const blacklistClient = (userID) => {
    axios
      .patch(`http://localhost:5000/api/v1/admin/client/${userID}`)
      .then((response) => {
        ...
        <Table>
          ...
          >
            <TableRow>
              ...
              <HeadTableCell>Blacklist Status</HeadTableCell>
            ...
            {data
              ? data.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <BodyTableCell>
                      {item.isBlackListed == true ? "true" : "false"}
                    </BodyTableCell>

                    <BodyTableCell>
                      <Button
                        // disableRipple
                        onClick={() => blacklistClient(item._id)}
                        sx={{
                          p: 1,
